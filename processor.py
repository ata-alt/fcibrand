...
def trim_whitespace(img: Image.Image, tolerance: int = 30) -> Image.Image:
    """
    Trims background using alpha channel if available, rembg segmentation if not.
    tolerance param kept for API compatibility but no longer used.
    """
    rgba      = img.convert("RGBA")
    alpha_min = min(rgba.split()[3].getdata())

    if alpha_min < 250:
        # Real alpha exists — trim using it directly
        logger.info(f"  Trim path: ALPHA | alpha_min: {alpha_min}")
        bbox = rgba.split()[3].getbbox()
    else:
        # Flat RGB — use rembg to generate alpha mask
        logger.info(f"  Trim path: REMBG | no alpha found (alpha_min: {alpha_min})")
    ...
    if bbox:
        ...
        logger.info(f"  Trimmed | bbox: {bbox} | "
                    f"before: {img.size} | "
                    f"after: {(bbox[2]-bbox[0], bbox[3]-bbox[1])}")
        return rgba.crop(bbox)

    logger.info("  Nothing to trim — empty bbox")
    return rgba
